...
var toolbar = {
  ...
  initToolbar: function initToolbar() {
    ...
    var addElementBtn = document.querySelector(".add_element");
    addElementBtn.addEventListener("click", toolbar.handleAddElementBtn);
    var deleteElementBtn = document.querySelector(".delete_element");
    deleteElementBtn.addEventListener("click", toolbar.handleRemoveElementBtn); // ================================= //
    // ======== COLOR SELECTOR ======== //

    var colorModButton = document.querySelector(".elem_mod_color");
    colorModButton.addEventListener("input", toolbar.handleColorChange); // =================================== //
    // ======== ELEMENT SELECTOR ======== //

    var elementSelector = document.querySelector("#element_selector");
    elementSelector.addEventListener("change", toolbar.handleElementSelector);
    toolbar.elemSelector = elementSelector; // ====================================== //
    // ======== ELEMENT ROUND RANGE ======== //

    var rangeElement = document.querySelector("#range_round");
    rangeElement.addEventListener("change", shapes.handleRangeRound);
    var listedElements = document.querySelectorAll(".list_item");

    var _iterator = _createForOfIteratorHelper(listedElements),
        _step;

    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var elementLi = _step.value;
  ...
  // ===================================== //
  // ======== ADD ELEMENT BUTTON ======== //
  handleAddElementBtn: function handleAddElementBtn(evt) {
    if (Object.entries(elements.allElements).length > 0) {
      elements.unselectAllElements();
    }

    ;
    var templateElement = document.querySelector(".new_element_tpl");

    if (Object.entries(elements.allElements).length < elements.maxElementNb) {
      var newElement = templateElement.content.cloneNode(true);
      ...
      var elementContainer = document.querySelector(".elements_container");
  ...
  handleRemoveElementBtn: function handleRemoveElementBtn(evt) {
    // remove element from dom :
    var elementContainer = document.querySelector(".elements_container");
    var elementToRemoveindex = elements.elemToModify.dataset.indexNumber;
    var elementToRemove = elements.elemToModify.closest(".element_content");
    elementContainer.removeChild(elementToRemove); // remove element from select :

    toolbar.removeElementFromSelect(elementToRemoveindex); // remove element from list :

    toolbar.removeElementFromList(elementToRemoveindex);
  },
  // =================================== //
  // ======== ELEMENT SELECTOR ======== //
  addElementToSelect: function addElementToSelect(elementName, elementValue) {
    var newElementToSelect = document.createElement("option");
  ...
  removeElementFromSelect: function removeElementFromSelect(elementToRemoveindex) {
    var selectElement = document.querySelector("#element_selector");
  ...
  handleElementSelector: function handleElementSelector(evt) {
    elements.elemToModify = elements.allElements["element-" + evt.target.value];
    elements.selectElement();
  },
  // ================================= //
  // ======== COLOR SELECTOR ======== //
  handleColorChange: function handleColorChange(evt) {
    var colorSelected = evt.target.value;
  ...
  // =============================== //
  // ======== ELEMENT LIST ======== //
  addElementToList: function addElementToList(elementIndex) {
    var listContainer = document.querySelector("#element_list ul");
    var newListedElement = document.createElement("li");
  ...
  removeElementFromList: function removeElementFromList(elementIndex) {
    var listContainer = document.querySelector("#element_list ul");
    var allListedElements = listContainer.querySelectorAll("li");
    var liElementIndex;
    var liElementToRemove;

    for (var oneElement in allListedElements) {
  ...
  handleClickOnList: function handleClickOnList(evt) {
...
},{}]},{},["../../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","assets/js/components/toolbar.js"], null)
//# sourceMappingURL=/toolbar.81876f3c.js.map